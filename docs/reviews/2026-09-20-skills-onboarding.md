...
The README leads with a short interactive GitHub command for `guildhall-quest`.
The installation guide provides explicit single-host copy commands and the
install → list → invoke → refresh → remove path. Guildhall remains independent of IDD; only its own skill is installed.
Plugin resources and versions are unchanged.

## Remote lifecycle observations

Public source: `GrillerGeek/guildhall`. Both `skills@1.5.25` and `skills@1.7.0`
...
3. `update guildhall-quest --project --yes` refreshed the current source and retained
   expected bundle content and an unrelated fixture skill.
4. Repeating explicit `add --skill guildhall-quest --agent <host> --copy --yes`
...
Local package probes passed on both installer versions: four cases per version
cover explicit bundle and repository-root discovery for Codex and Claude. Every
installed byte/mode matched and copies survived source removal. Receipts:
`guildhall-install-2oid81su/report.json` (1.5.25) and
`guildhall-install-xh0l647x/report.json` (1.7.0), under the temporary parent above.

## Directory discovery

GitHub shorthand discovery succeeds. However, `skills find` did not return the
matching GrillerGeek skill, and the page at
`https://skills.sh/grillergeek/guildhall/guildhall-quest` returned HTTP 200 with rendered
...
Native agent validation reports 19 agents with no errors or warnings;
portable metadata/bundle validation and all **16 unit tests** pass.
