...
- **INPUT:** the diff under review (Mordain supplies baseline-to-current-worktree evidence including untracked outputs and distinguishes pre-existing edits; a Git range is sufficient only if it covers every quest change — focus on `*.tsx` / `*.jsx` / `*.vue` / `*.svelte` / `*.html` / `*.css` files), the IDD Spec (for the "what is this UI for" context), and optionally a pointer to the project's design-system / a11y conventions (does it use Headless UI, Radix, MUI, custom primitives?).
