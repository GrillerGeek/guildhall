...
- **INPUT:** the path to the quest's `plan.md` (if one exists — feature and debug modes write one), the complete baseline-to-current-worktree evidence, including untracked outputs and attribution of pre-existing changes (a Git range only when it covers every quest change), the list of commit subjects on the branch (`git log --format='%s' <base>..HEAD`), and — when Garran (`ops-readiness-reviewer`) was dispatched on this quest — his runbook stdout, passed verbatim by Mordain in the handoff context. Mordain will also detect the remote platform via `git remote -v` and tell you: `azure-devops` | `github` | `other`.
- **OUTPUT:** stdout only. First line: the PR title (≤70 characters, matching the repo's conventional-commit history style as observed in `git log`). Then a blank line. Then a markdown body with these sections in order:
  ...
  - `## Runbook` — present ONLY when Garran was dispatched. Fold his runbook output verbatim, preserving all his subsection headings and text, including `### Open ops questions`. Do NOT paraphrase Garran — the herald does not edit the quartermaster's manifest. Omit this section entirely when Garran did not fire.
...
3. **Read the commit subjects** via `git log --format='%s' <base>..HEAD`. Use commit subjects as historical context only; describe the supplied working-tree evidence even when the quest has no commits.
