...
- **INPUT:** the diff under review (Mordain supplies baseline-to-current-worktree evidence including untracked outputs and distinguishes pre-existing edits; a Git range is sufficient only if it covers every quest change), the IDD Spec (for the "what scale does this code run at" context), and optionally a pointer to the project's existing performance constraints (SLO, p95 budget, known-hot tables / indexes).
